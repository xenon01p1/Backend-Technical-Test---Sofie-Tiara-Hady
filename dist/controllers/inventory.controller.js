...
export const getInventory = async (req, res, next) => {
    ...
    try {
        ...
        logRequest('GET', '/inventory', true, `Retrieved inventory records=${result.data.length}`, startTime);
        res.status(200).json(result);
    }
    catch (error) {
        logRequest('GET', '/inventory', false, 'Failed to retrieve inventory', startTime);
        next(error);
    }
};
