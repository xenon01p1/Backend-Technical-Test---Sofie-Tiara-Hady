...
export const createPurchaseOrder = async (
  ...
  try {
    ...
    const [poResult] = await connection.execute<ResultSetHeader>(
      `
...
export const getPurchaseOrderById = async (
  ...
  const [poRows] = await db.execute<PurchaseOrderRow[]>(
    `
      SELECT
        id,
        po_number,
  ...
  const [itemRows] = await db.execute<PurchaseOrderItemRow[]>(
    `
      SELECT
        id,
        purchase_order_id,
  ...
  return {
    ...po,
    items: itemRows as PurchaseOrderItem[],
  };
};

export const getPurchaseOrdersByCursor = async (
  ...
  const [rows] = await db.execute<PurchaseOrderRow[]>(
    `
      SELECT
        po.id,
        po.po_number,
