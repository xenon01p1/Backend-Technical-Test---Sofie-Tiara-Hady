...
const generateGoodsReceiptNumber = (): string => {
...
export const createGoodsReceipt = async (
  ...
  if (purchaseRequest.requested_by !== userId) {
    throw new AppError(
      403,
      'GOODS_RECEIPT_FORBIDDEN',
      'You are not allowed to create a Goods Receipt for this Purchase Order.'
    );
  }
  ...
  return goodsReceiptRepository.createGoodsReceipt(
    goodsReceiptNumber,
    data
  );
};
...
export const getGoodsReceiptById = async (
  ...
  const goodsReceipt =
    await goodsReceiptRepository.getGoodsReceiptById(id);
  ...
  if (
    ...
    throw new AppError(
      403,
      'GOODS_RECEIPT_FORBIDDEN',
      'You are not allowed to access this Goods Receipt.'
    );
  }
